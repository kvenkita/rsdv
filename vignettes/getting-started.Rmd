...
The optional `.n` column sets how many rows to generate per condition; supply multiple rows to request several conditions at once. `.n` values must be positive whole numbers. Conditioning on numerical columns is not supported (exact equality is ill-defined for continuous values). Metadata constraints attached to the synthesizer are also honoured during conditional sampling, so any row that violates them is rejected just like in plain `sample()`.
...
By default `nndr()` z-scores each numerical column using the real-data mean and standard deviation before the Euclidean distance is computed. Without this, a single large-scale column (income in dollars, for instance) dominates the distance and the score moves with measurement units rather than with row similarity. Pass `normalize = FALSE` to recover the unscaled behaviour explicitly.
...
For continuous numerical columns, exact `==` is almost never satisfied by the copula sampler — use the `tolerance` argument on `equality_constraint()` (or a tight band of `inequality_constraint()`s) instead. Rows containing `NA` are treated as failing equality/inequality checks, so they are rejected by the sampler rather than producing phantom NA rows in the output.
